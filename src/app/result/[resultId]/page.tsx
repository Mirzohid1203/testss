    ...
    if (!result) {
        return (
            <div className="flex h-screen flex-col items-center justify-center bg-white dark:bg-gray-950 px-4 text-center transition-colors duration-300">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t.result.not_found}</h2>
                <Link href="/dashboard" className="mt-4 text-blue-600 dark:text-blue-500 underline">{t.test.backDashboard}</Link>
            </div>
        );
    }
        ...
        <ProtectedRoute>
            <main className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
                <motion.div
                    ...
                    className="overflow-hidden rounded-3xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/50 shadow-xl dark:shadow-2xl backdrop-blur-xl transition-colors duration-300"
                >
                    <div className={`h-4 ${isPassed ? "bg-emerald-500" : "bg-red-500"}`} />

                    <div className="p-8 text-center sm:p-12">
                        <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 shadow-inner">
                            {isPassed ? (
                        ...
                        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-3">
                            <div className="rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 p-6 backdrop-blur-sm">
                                <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t.result.score}</div>
                                <div className="mt-1 text-3xl font-black text-gray-900 dark:text-white">
                                    {result.score} <span className="text-xl font-normal text-gray-400 dark:text-gray-600">/ {result.total}</span>
                                </div>
                            </div>
                            <div className="rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 p-6 backdrop-blur-sm">
                                <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t.result.percentage}</div>
                                <div className={`mt-1 text-3xl font-black ${isPassed ? "text-emerald-500" : "text-red-500"}`}>
                                    {percentage}%
                                </div>
                            </div>
                            <div className="rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 p-6 backdrop-blur-sm">
                                <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t.result.timeSpent}</div>
                                <div className="flex items-center justify-center mt-1 text-3xl font-black text-gray-900 dark:text-white">
                        ...
                        <div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
                            <Link
                                href="/dashboard"
                                className="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800/50 px-6 py-4 font-bold text-gray-700 dark:text-white transition-all hover:bg-gray-200 dark:hover:bg-gray-800 sm:w-auto active:scale-95"
                            >
                            ...
                            <button
                                ...
                                className="flex w-full items-center justify-center gap-2 rounded-xl bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/10 px-6 py-4 font-bold text-gray-700 dark:text-white transition-all hover:bg-gray-200 dark:hover:bg-white/20 sm:w-auto active:scale-95"
                            >
                ...
                {showReview && (
                    ...
                    >
                        <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight font-outfit uppercase text-center mb-8">
                                ...
                                >
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-lg font-bold text-gray-900 dark:text-white mb-4 leading-relaxed">{q.question}</p>
                                            ...
                                            <div className="grid grid-cols-1 gap-3">
                                                {q.options.map((opt, optIdx) => {
                                                    ...
                                                    return (
                                                        <div 
                                                            key={optIdx} 
                                                            className={`flex items-center gap-3 rounded-xl border p-3 text-sm font-medium transition-all ${borderColor} ${bgColor} ${isCorrectOpt ? "text-emerald-600 dark:text-emerald-400" : isUserSelected ? "text-red-600 dark:text-red-400" : "text-gray-600 dark:text-gray-400"}`}
                                                        >
                                                ...
                                                <div className="mt-4 flex flex-wrap gap-3 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 p-4">
                                            ...
                                            {isCorrect && (
                                                <div className="mt-4 flex items-center gap-2 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 p-4">
